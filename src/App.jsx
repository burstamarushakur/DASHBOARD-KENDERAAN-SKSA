...
export default function App() {
  ...
  const tabs = [
    ...
    { id: 'sokongan', label: 'Kenderaan Staf Sokongan', icon: Wrench },
    { id: 'ppki', label: 'Kenderaan Ibubapa/Penjaga Murid PPKI', icon: Users },
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'sokongan':
        return (
          <VehicleModule
            title="Senarai Kenderaan Staf Sokongan"
            kategori="SOKONGAN"
            key="SOKONGAN"
          />
        );
      ...
      case 'ppki':
        return (
          <VehicleModule
            title="Senarai Kenderaan Ibubapa/Penjaga Murid PPKI"
            kategori="PPKI"
            key="PPKI"
          />
        );
  ...
  const activeLabel = tabs.find(t => t.id === activeTab)?.label || 'Laman Utama';
      ...
      >
        ...
        <div className="p-6 flex flex-col items-center border-b border-slate-800">
          ...
          <div className="text-center">
            <h1 className="text-[10px] font-bold tracking-widest text-cyan-400 uppercase">
              Dashboard Kenderaan
            </h1>
      ...
      <main className="flex-1 flex flex-col bg-slate-950 overflow-hidden">
        ...
        <header className="h-16 border-b border-slate-800 bg-slate-900/30 flex items-center justify-between px-4 lg:px-8 shrink-0">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <h2 className="text-lg font-semibold text-white hidden sm:block truncate">
              {activeLabel}
            </h2>

            <span className="text-[11px] bg-slate-800 px-3 py-1 rounded-full text-slate-400 border border-slate-700 hidden md:block shrink-0">
              {currentDate}
            </span>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="relative hidden md:block">
              <input
                type="text"
                placeholder="Carian No. Plat (e.g. JNB 4432)"
                className="bg-slate-800 border border-slate-700 text-xs rounded-lg py-2 px-10 w-64 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all text-slate-200"
              />
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-9 h-9 rounded-full bg-cyan-500 flex items-center justify-center font-bold text-slate-950 shrink-0">
                AD
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 p-4 lg:p-8 overflow-y-auto space-y-6">
          ...
          <footer className="flex items-center justify-between text-[10px] text-slate-500 bg-slate-900/20 p-4 rounded-xl border border-slate-800 mt-auto shrink-0">
            ...
            <div className="font-semibold text-right max-w-[50%]">
              &copy; 2026 UNIT ICT SKSA - SISTEM PENDAFTARAN KENDERAAN
            </div>
          </footer>
        </div>
      </main>
